...
const Silk: React.FC<SilkProps> = ({
  ...
  // читаем CSS-переменную --silk-color
  const silkColor = getComputedStyle(document.documentElement)
  ...
  const uniforms: SilkUniforms = useMemo(
    () => ({
      ...
      uColor: { value: new Color(silkColor || "#f5e8bf") },
  ...
  useEffect(() => {
    const observer = new MutationObserver(() => {
      const newColor = getComputedStyle(document.documentElement)
        .getPropertyValue("--silk-color")
        .trim();
      uniforms.uColor.value = new Color(newColor || "#f5e8bf");
    });
